    ...
    @foreach ($names as $index=>$name)
        @php
            if (!$name->VIAF_id && !$name->WIKIDATA_id && !$name->VIAF_multiple) {
                $color_class = 'alert-danger border-danger alert-box';
                $auto_success = 'false';
            } elseif (!$name->VIAF_id) {
                $color_class = 'alert-warning border-warning alert-box';
                $auto_success = 'pending';
            } else {
                $color_class = 'alert-success border-success alert-box';
                $auto_success = 'true';
            }
        @endphp
        <div class="card my-1" id="card{{$name->id}}">
            <div class="card-header">
                <h5 class="mb-0 d-flex flex-row">
                    <button type="button" class="btn view w-100 text-dark text-left {{$color_class}}" data-toggle="collapse" data-target="#collapse{{$index}}" aria-expanded="true" aria-controls="{{$index}}">
            ...
            <div id="collapse{{$index}}" class="collapse" aria-labelledby="{{$index}}" data-parent="#accordion">
                <div class="card-body bg-custom-light-gray">
                    <div class="alert alert-warning manual-change" style="display:none;">
                        <p class="mb-0">
                            @lang('You have manually updated the data. Don\'t forget to save!')
                        </p>
                    </div>
